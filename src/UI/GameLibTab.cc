...
// może troszkę myloncje jest tutaj to gameID ale teraz niewiem jak to nazwać innaczej
void GameLibTab::updateGameDLCsList(int gameID) {
  ...
  for (auto ownedProd : tempOwnedDlCs) {
    if (ownedProd.first == gameID) {
      ...
      dlcList->add(tempGame.getDLCs()[ownedProd.second-1].getTitle().c_str());
...
void GameLibTab::userSearch(std::string value) {
  ...
  if (tempVect.size()) {
    ...
    for (auto ownedGame:tempOwnedGames) {
        gameList->add(ownedGame.getTitle().c_str());
    }
  }
}
